...
extension UIViewController {
  ...
  @IBAction func presentLeftMenuViewController() {
    print("Present left menu vc in the library")
    print("Side menu vc is....\(sideMenuViewController)")
...
@objc protocol SSASideMenuDelegate {
  ...
  @objc optional func sideMenuOffsetDidChange(_ sideMenu: SSASideMenu, newOffset: CGFloat)
...
class SSASideMenu: UIViewController, UIGestureRecognizerDelegate {
  ...
  fileprivate var accessible: Bool = true
  ...
  func isMenuShown() -> Bool {
    return (self.leftMenuViewController?.view.isHidden)!
  }
  ...
  fileprivate func animateMenuViewControllerCompletion(_ side: SSASideMenuSide, menuViewController: UIViewController) {
    print("Animate menu vc completion")
    if !visible {
  ...
  fileprivate func presentMenuViewContainerWithMenuViewController(_ menuViewController: UIViewController?) {
    print("Present mneu view container with....")
    menuViewContainer.transform = CGAffineTransform.identity
  ...
  @objc fileprivate func makeAccessible() {
    self.accessible = true
  }
  
  @objc fileprivate func makeInAccessible() {
  ...
  func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
    ...
    if gestureRecognizer is UIPanGestureRecognizer && !visible {
      ...
      case .edge:
        ...
        if (point.x < 20.0 || point.x > view.frame.size.width - 20.0) && accessible { return true }
        else { return false }
      }
    }
  ...
  func panGestureRecognized(_ recognizer: UIPanGestureRecognizer) {
    ...
    if recognizer.state == .changed {
      ...
      delegate?.sideMenuOffsetDidChange?(self, newOffset: contentViewContainer.frame.origin.x)
